...
src_prepare() {
	...
	if use nginx_modules_http_spdy; then
		# epatch is broken: you cannot force -p1
		einfo "Applying patch.spdy-${SPDY_PATCH_VER}.txt…"
		patch -p1 -g0 -E --no-backup-if-mismatch -i "${DISTDIR}"/patch.spdy-${SPDY_PATCH_VER}.txt || die "SPDY patch has not been applied"
	fi

	if use nginx_modules_http_upload; then
		cd "${WORKDIR}"
		epatch "${FILESDIR}"/nginx-1.3.14_upload_module.patch
		cd -
	fi
