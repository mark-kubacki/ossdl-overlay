# Copyright 2010-2013 W-Mark Kubacki, Mao Pu
...
REDIS_PIDFILE=${REDIS_PIDDIR}/redis2.4.pid
REDIS_DATAPATH=/var/lib/redis2.4
REDIS_LOGPATH=/var/log/redis
REDIS_LOGFILE=${REDIS_LOGPATH}/redis2.4.log
...
src_prepare() {
	...
	if use jemalloc ; then
		...
		sed -i -e "s/je_/${JEMALLOC_PREFIX}/" src/zmalloc.c src/zmalloc.h
	fi
	...
	# rename binaries to enable user to install Redis 2.4 alongside with later versions
	sed -i	-e 's:PRGNAME = redis-:PRGNAME = redis2.4-:g' \
		src/Makefile || die "renaming binaries in Makefile failed"
	# replace port 6379 by ${REDIS_PORT}
	for F in $(grep -rl '6379' ./*); do
		sed -i	-e "s:6379:${REDIS_PORT}:g" \
			$F
	done
...
src_install() {
	...
	sed -r \
		...
		<redis.conf \
		>redis.conf.gentoo
	newins redis.conf.gentoo redis2.4.conf
	use prefix || fowners redis:redis /etc/redis2.4.conf
	fperms 0644 /etc/redis2.4.conf

	sed \
		-e "s:redis.conf:redis2.4.conf:g" \
		-e "s:redis.pid:redis2.4.pid:g" \
		-e "s:/var/lib/redis:/var/lib/redis2.4:g" \
		<"${FILESDIR}/redis.confd" \
		>redis.confd.gentoo
	newconfd redis.confd.gentoo redis2.4
	sed \
		-e "s:redis.conf:redis2.4.conf:g" \
		-e "s:redis.pid:redis2.4.pid:g" \
		-e "s:redis-server:redis2.4-server:g" \
		-e "s:redis-cli:redis2.4-cli:g" \
		-e "s:/var/lib/redis:/var/lib/redis2.4:g" \
		-e "s:ing Redis:ing Redis 2.4:g" \
		<"${FILESDIR}/redis.initd-v2" \
		>redis.initd-v2.gentoo
	newinitd redis.initd-v2.gentoo redis2.4

	nonfatal dodoc 00-RELEASENOTES BUGS CONTRIBUTING COPYING README

	dobin src/redis2.4-cli
	dosbin src/redis2.4-benchmark src/redis2.4-server src/redis2.4-check-aof src/redis2.4-check-dump
	fperms 0750 /usr/sbin/redis2.4-benchmark
