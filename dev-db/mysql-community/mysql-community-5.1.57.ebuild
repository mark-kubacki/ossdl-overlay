...
SRC_URI="${SERVER_URI}
	http://static.ossdl.de/mirror/mysql-extras-${MY_EXTRAS_VER}.tar.bz2"
mysql_version_is_at_least "5.1.12" \
&& [[ -n "${PBXT_VERSION}" ]] \
&& SRC_URI="${SRC_URI} pbxt? ( http://www.primebase.org/download/pbxt-${PBXT_VERSION}.tar.gz )"

mysql_version_is_at_least "5.1.15" \
&& [[ -n "${SPHINX_VERSION}" ]] \
&& SRC_URI="${SRC_URI} sphinx? ( http://sphinxsearch.com/files/sphinx-${SPHINX_VERSION}.tar.gz )"
...
pkg_postinst() {
	...
	einfo "Version 5.1 becomes obsolete. Please upgrade to version 5.5."
}
