...
IUSE="cgi fastcgi postgres sqlite enscript silvercity rbac rbl"
...
src_compile() {
	if use rbac ; then
		epatch ${FILESDIR}/rbac.patch
	fi
	if use rbl ; then
		epatch ${FILESDIR}/trac-0.9.6-rbl.patch
	fi
	epatch ${FILESDIR}/trac-0.9.5-ISO8601.patch
}
...
src_install () {
	...
	if use rbl ; then
		einfo "In order to activate RBL support for Trac you will have to add to your conf/trac.ini"
		einfo "[trac]Â"
		einfo "rbl = sbl-xbl.spamhaus.org,cbl.abuseat.org"
		einfo "... or whatever RBL you prefer."
	fi
